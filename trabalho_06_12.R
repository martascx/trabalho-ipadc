...
server <- function(input, output, session) {
  ...
  calculo_risco <- reactive({
    ...
    if (age < 30 | age > 120) {
      stop("A idade tem que estar compreendida entre os 30 e os 120 anos.")
    }
    
    if (tcl < 100 | tcl > 400) {
      stop("Os valores totais de colesterol tem que estar entre 100 e 400 mg/dl.")
    }
    
    if (hdl < 10 | hdl > 150) {
      stop("O colesterol HDL tem que estar entre 10 e 150 mg/dl.")
    }
  ...
  output$download_relatorio <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (!id %in% names(rv$d)) {
        df_hist <- data.frame(Nota="Sem histórico disponível")
      } else {
        historico <- rv$d[[id]]
        df_hist <- do.call(rbind, lapply(historico, as.data.frame))
        
        # Convert to POSIXct to sort chronologically
      ...
      grid.text(
        paste0("Paciente: ", input$patient_name,
      ...
      hist_table <- tableGrob(df_hist, rows=NULL, theme=ttheme_default())
      pushViewport(viewport(x=0.5, y=0.65, width=0.95, height=0.55))
      ...
      grid.newpage()
      grid.text("Evolução Temporal",
                y=0.97, gp=gpar(fontsize=18, fontface="bold"))
      
      # Convert df_hist for plotting
      if (!"Nota" %in% names(df_hist)) {
        par(mfrow=c(3,1))  # podemos ter 3 gráficos numa página
        
        plot(df_hist$datetime, df_hist$cdv, type="b",
             col="#E63946", pch=19, lwd=3,
             xlab="Data", ylab="Risco (%)",
             main="Evolução do Risco Cardiovascular")
        
        plot(df_hist$datetime, df_hist$pas, type="b",
             col="#1D3557", pch=19, lwd=3,
             xlab="Data", ylab="PAS",
             main="Pressão Arterial Sistólica")
        
        plot(df_hist$datetime, df_hist$colesterol_total, type="b",
             col="#2A9D8F", pch=19, lwd=3,
             xlab="Data", ylab="Colesterol",
             main="Colesterol Total")
      } else {
        grid.text("Sem histórico suficiente para gráficos.",
                  y=0.5, gp=gpar(fontsize=14, col="grey40"))
      }
