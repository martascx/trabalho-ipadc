...
server <- function(input, output, session) {
  ...
  output$hdl_high <- renderUI({ if (as.numeric(input$hdl) > 60) HTML(paste("<h6 style=\"color:red;\">Valor de Colesterol HDL elevado!")) })
  ...
  output$download_relatorio <- downloadHandler(
    ...
    content = function(file) {
      ...
      pushViewport(viewport(x=0.5, y=0.60, width=0.9, height=0.40))
      grid.draw(t)
      popViewport()
      grid.text("Último Registo", y=0.88, gp=gpar(fontsize=18, fontface="bold"))
